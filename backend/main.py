...
STUDENT_FIELDS = [
    ...
    {"key": "interests", "label": "관심분야", "required": False},
    {"key": "education", "label": "학력", "required": False},
    {"key": "introduction", "label": "자기소개", "required": False},
    {"key": "campus", "label": "캠퍼스", "required": False},
]

def auto_match_columns(excel_columns):
    """Excel 컬럼과 시스템 필드를 자동 매칭"""
    # 키워드 기반 매칭 규칙
    match_rules = {
        "name": ["이름", "성명", "name"],
        "birth_date": ["생년월일", "생일", "birth", "출생"],
        "gender": ["성별", "gender", "sex"],
        "phone": ["전화", "휴대폰", "연락처", "phone", "mobile", "핸드폰"],
        "email": ["이메일", "email", "메일"],
        "address": ["주소", "address", "거주지"],
        "interests": ["관심", "interest", "분야"],
        "education": ["학력", "학교", "education", "졸업"],
        "introduction": ["자기소개", "소개", "intro"],
        "campus": ["캠퍼스", "campus", "지원", "센터"],
    }

    mapping = {}
    for field_key, keywords in match_rules.items():
        for excel_col in excel_columns:
            excel_col_lower = str(excel_col).lower().replace(" ", "").replace("\n", "")
            for keyword in keywords:
                if keyword.lower() in excel_col_lower:
                    mapping[field_key] = excel_col
                    break
            if field_key in mapping:
                break
    return mapping

@app.post("/api/students/preview-excel")
async def preview_excel(file: UploadFile = File(...)):
    """Excel 파일 미리보기 - 컬럼 추출 및 자동 매핑"""
    ...
    try:
        ...
        auto_mapping = auto_match_columns(excel_columns)

        # 샘플 데이터 (첫 3행)
        sample_data = df.head(3).fillna('').astype(str).to_dict('records')

        return {
            "excel_columns": excel_columns,
            "system_fields": STUDENT_FIELDS,
...
async def upload_excel(file: UploadFile = File(...), course_code: str = None, mapping: str = None):
    """Excel 파일로 학생 일괄 등록 (매핑 지원)"""
    if not file.filename.endswith(('.xlsx', '.xls')):
        raise HTTPException(status_code=400, detail="Excel 파일만 업로드 가능합니다")
    ...
    try:
        ...
        if mapping:
            ...
            # 기본 매핑 (기존 호환성 유지)
            field_mapping = auto_match_columns(df.columns.tolist())

        conn = get_db_connection()
        cursor = conn.cursor()

        print(f"[UPLOAD] course_code: {course_code}, mapping: {mapping}")
        ...
        course_name = ''
        if course_code:
            cursor.execute("SELECT name FROM courses WHERE code = %s", (course_code,))
            course_result = cursor.fetchone()
            if course_result:
                course_name = course_result[0]

        # 기존 학생 목록 조회 (중복 체크용: 이름, 전화번호, 성별)
        cursor.execute("SELECT name, phone, gender FROM students")
        existing_students = set()
        for row in cursor.fetchall():
            phone_normalized = str(row[1] or '').replace('-', '').replace(' ', '')
            existing_students.add((str(row[0] or '').strip(), phone_normalized, str(row[2] or '').strip()))

        success_count = 0
        duplicate_list = []  # 중복 학생 목록
        skipped_list = []  # 건너뛴 학생 목록 (이름 없음)
        registered_list = []  # 등록된 학생 목록
        total_count = len(df)
        ...
        for idx, row in df.iterrows():
            try:
                ...
                def get_value(field_key):
                    col_name = field_mapping.get(field_key)
                ...
                name = get_value('name')
                if not name:  # 이름은 필수
                    skipped_list.append(f"행 {idx+2}: 이름 없음")
                    continue
                ...
                introduction = get_value('introduction')
                campus = get_value('campus')

                # 캠퍼스가 빈칸이면 과정명으로 자동 입력
                if not campus and course_name:
                    campus = course_name

                # 생년월일이 빈칸이면 기본 비밀번호 설정
                password = ''
                if not birth_date:
                    password = 'kdt2025'

                # 중복 체크 (이름, 전화번호, 성별)
                student_key = (name, phone_normalized, gender)
                if student_key in existing_students:
                    duplicate_list.append({"name": name, "phone": phone, "gender": gender})
                    continue
                ...
                query = """
                    INSERT INTO students
                    (code, name, birth_date, gender, phone, email, address, interests, education, introduction, campus, course_code, password)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                """

                cursor.execute(query, (
                    code, name, birth_date, gender, phone, email,
                    address, interests, education, introduction, campus, course_code, password
                ))

                existing_students.add(student_key)
                registered_list.append({"code": code, "name": name})
                next_num += 1
                success_count += 1

            except Exception as e:
                skipped_list.append(f"행 {idx+2}: {str(e)}")

        conn.commit()
        conn.close()

        # 상세 메시지 생성
        msg_parts = [f"총 {total_count}명"]
        if success_count > 0:
            msg_parts.append(f"{success_count}명 등록")
        if duplicate_list:
            dup_names = ', '.join([d['name'] for d in duplicate_list[:5]])
            if len(duplicate_list) > 5:
                dup_names += f" 외 {len(duplicate_list)-5}명"
            msg_parts.append(f"{len(duplicate_list)}명 중복({dup_names})")
        if skipped_list:
            msg_parts.append(f"{len(skipped_list)}명 건너뜀")
        ...
        return {
            ...
            "registered_list": registered_list[:10],
...
async def download_template():
    """학생 등록 템플릿 다운로드"""
    template_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "student_template.xlsx")
...
async def create_instructor_code(data: dict):
    ...
    try:
        ...
        # dashboard_charts 컬럼이 없으면 추가
        cursor.execute("SHOW COLUMNS FROM instructor_codes LIKE 'dashboard_charts'")
        if not cursor.fetchone():
            cursor.execute("ALTER TABLE instructor_codes ADD COLUMN dashboard_charts TEXT DEFAULT NULL")
            conn.commit()
        ...
        dashboard_charts_json = json.dumps(data.get('dashboard_charts', [])) if data.get('dashboard_charts') else None

        query = """
            INSERT INTO instructor_codes (code, name, type, permissions, menu_permissions, default_screen, dashboard_charts)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """
        cursor.execute(query, (data['code'], data['name'], data['type'], permissions_json, menu_permissions_json, default_screen, dashboard_charts_json))
        conn.commit()
...
async def update_instructor_code(code: str, data: dict):
    ...
    try:
        ...
        query = """
            ...
            SET name = %s, type = %s, permissions = %s, menu_permissions = %s, default_screen = %s, dashboard_charts = %s
            WHERE code = %s
        """
        cursor.execute(query, (data['name'], data['type'], permissions_json, menu_permissions_json, default_screen, dashboard_charts_json, code))
        conn.commit()
        return {"code": code}
    finally:
        conn.close()
...
def ensure_instructor_courses_table(cursor):
    """instructor_courses 테이블이 없으면 생성"""
    try:
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS instructor_courses (
                ...
                UNIQUE KEY uk_instructor_course (instructor_code, course_code),
...
async def get_instructors(search: Optional[str] = None):
    ...
    try:
        ...
        # instructor_courses 테이블에서 과정 배정 정보 조회
        ensure_instructor_courses_table(cursor)
        conn.commit()
        for inst in instructors:
            cursor.execute(
                "SELECT course_code FROM instructor_courses WHERE instructor_code = %s",
                (inst['code'],)
            )
            inst['course_codes'] = [row['course_code'] for row in cursor.fetchall()]
...
async def create_instructor(data: dict):
    ...
    try:
        ...
        # motto, affiliation 컬럼 자동 생성
        try:
            cursor.execute("ALTER TABLE instructors ADD COLUMN motto TEXT")
            conn.commit()
        except:
            pass
        try:
            cursor.execute("ALTER TABLE instructors ADD COLUMN affiliation VARCHAR(255)")
            conn.commit()
        except:
            pass

        query = """
            INSERT INTO instructors (code, name, phone, major, instructor_type, email, profile_photo, attachments, motto, affiliation)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        """
        cursor.execute(query, (
            data['code'], data['name'], data.get('phone'),
            data.get('major'), data.get('instructor_type'), data.get('email'),
            data.get('profile_photo'), data.get('attachments'), data.get('motto'), data.get('affiliation')
        ))

        # 과정 배정 저장
        ensure_instructor_courses_table(cursor)
        course_codes = data.get('course_codes', [])
        if course_codes and course_codes != ['ALL']:
            for cc in course_codes:
                cursor.execute(
                    "INSERT IGNORE INTO instructor_courses (instructor_code, course_code) VALUES (%s, %s)",
                    (data['code'], cc)
                )
    ...
    try:
        ...
        query = """
            ...
            SET name = %s, phone = %s, major = %s, email = %s,
                profile_photo = %s, attachments = %s, motto = %s, affiliation = %s
            WHERE code = %s
        """
        cursor.execute(query, (
            name, phone, major, email, profile_photo, attachments, motto, affiliation, code
        ))

        # 과정 배정 업데이트 (DELETE + INSERT)
        ensure_instructor_courses_table(cursor)
        course_codes = data.get('course_codes')
        if course_codes is not None:
            cursor.execute("DELETE FROM instructor_courses WHERE instructor_code = %s", (code,))
            if course_codes and course_codes != ['ALL']:
                for cc in course_codes:
                    cursor.execute(
                        "INSERT INTO instructor_courses (instructor_code, course_code) VALUES (%s, %s)",
                        (code, cc)
                    )
...
async def delete_instructor(code: str):
    ...
    try:
        ...
        # FK RESTRICT 참조 해제: consultations, subjects, timetables
        try:
            cursor.execute("UPDATE consultations SET instructor_code = NULL WHERE instructor_code = %s", (code,))
        except Exception:
            pass
        try:
            cursor.execute("UPDATE subjects SET main_instructor = NULL WHERE main_instructor = %s", (code,))
            cursor.execute("UPDATE subjects SET assistant_instructor = NULL WHERE assistant_instructor = %s", (code,))
            cursor.execute("UPDATE subjects SET reserve_instructor = NULL WHERE reserve_instructor = %s", (code,))
        except Exception:
            pass
        try:
            cursor.execute("UPDATE timetables SET instructor_code = NULL WHERE instructor_code = %s", (code,))
        except Exception:
            pass

        # 기타 관련 테이블 정리
        for table in ['class_notes', 'training_logs', 'exam_bank', 'online_exams']:
            try:
                cursor.execute(f"UPDATE {table} SET instructor_code = NULL WHERE instructor_code = %s", (code,))
            except Exception:
                pass
...
async def delete_course(code: str):
    ...
    try:
        ...
        # 1. 학생의 course_code를 NULL로 설정 (학생 데이터는 유지)
        cursor.execute("UPDATE students SET course_code = NULL WHERE course_code = %s", (code,))

        # 2. 프로젝트의 course_code를 NULL로 설정 (프로젝트 데이터는 유지)
        cursor.execute("UPDATE projects SET course_code = NULL WHERE course_code = %s", (code,))
...
async def approve_student_registration(registration_id: int, data: dict):
    ...
    try:
        ...
        # 프론트엔드에서 과정 코드가 전달되면 해당 값 사용, 아니면 신청 시 선택한 값 사용
        final_course_code = data.get('course_code') or registration['course_code']
...
# 앱 시작 시 RAG 백그라운드 초기화 (서버 즉시 시작)
import threading

def _init_rag_background():
    """백그라운드 스레드에서 RAG 초기화 (서버 시작 블로킹 방지)"""
    try:
        init_rag()
        print("[OK] RAG 백그라운드 초기화 완료 - RAG 기능 사용 가능")
    except:
        print("[WARN] RAG 초기화 실패 - RAG 기능 비활성화됨")

_rag_thread = threading.Thread(target=_init_rag_background, daemon=True)
_rag_thread.start()
print("[INFO] RAG 시스템 백그라운드 로딩 시작 (서버는 즉시 사용 가능)")
...
async def system_check():
    ...
    try:
        ...
        result['db'] = {"success": True, "response_time": db_time, "host": DB_CONFIG['host'], "database": DB_CONFIG['db']}
    except Exception as e:
        db_time = int((time.time() - db_start) * 1000)
        result['db'] = {"success": False, "response_time": db_time, "error": str(e)}
        conn = None
        cursor = None
    ...
    try:
        ...
        ftp.login(FTP_CONFIG['user'], FTP_CONFIG['passwd'])
        ftp.quit()
        ftp_time = int((time.time() - ftp_start) * 1000)
        result['ftp'] = {"success": True, "response_time": ftp_time, "host": FTP_CONFIG['host']}
    except Exception as e:
        ftp_time = int((time.time() - ftp_start) * 1000)
        result['ftp'] = {"success": False, "response_time": ftp_time, "error": str(e)}

    # 3. 테이블별 레코드 수
    tables_to_check = [
        'students', 'instructors', 'courses', 'subjects', 'consultations',
        'timetables', 'training_logs', 'projects', 'team_activity_logs',
        'class_notes', 'holidays', 'notices', 'instructor_codes',
        'exam_bank', 'exam_questions', 'online_exams', 'online_exam_participants',
        'student_registrations', 'system_settings', 'course_subjects',
        'instructor_courses', 'instructor_notes', 'db_management_logs'
    ]
    table_counts = {}
    try:
        if not conn:
        ...
        for table in tables_to_check:
            try:
                cursor.execute(f"SELECT COUNT(*) as cnt FROM {table}")
                row = cursor.fetchone()
                table_counts[table] = row['cnt'] if isinstance(row, dict) else row[0]
            except Exception:
    ...
    # 4. AI 설정 상태
    ai_config = {"openai": False, "groq": False, "gemini": False}
    try:
        conn2 = get_db_connection()
        cursor2 = conn2.cursor()
        cursor2.execute("SELECT setting_key, setting_value FROM system_settings WHERE setting_key IN ('groq_api_key', 'gemini_api_key')")
    ...
    if os.getenv('GROQ_API_KEY', ''):
        ai_config['groq'] = True
    if os.getenv('GOOGLE_CLOUD_TTS_API_KEY', ''):
        ai_config['gemini'] = True
    if os.getenv('OPENAI_API_KEY', ''):
        ai_config['openai'] = True
    result['ai_config'] = ai_config

    # 5. RAG 시스템 상태
    rag_status = {"available": False, "doc_count": 0}
    try:
        if vector_store_manager:
            doc_count = vector_store_manager.count_documents()
            rag_status = {"available": True, "doc_count": doc_count}
    except Exception:
        pass
    ...
    try:
        ...
        result['system_resources'] = {
            ...
            "memory": {
                "total_gb": round(sys_mem.total / 1024 / 1024 / 1024, 1),
                "used_gb": round(sys_mem.used / 1024 / 1024 / 1024, 1),
                "available_gb": round(sys_mem.available / 1024 / 1024 / 1024, 1),
...
async def get_server_services():
    ...
    try:
        ...
        if result.returncode != 0:
            raise HTTPException(status_code=500, detail="PM2 명령 실행 실패")
        ...
        for proc in processes:
            ...
            name = pm2_env.get("name", proc.get("name", "unknown"))
            uptime_ms = pm2_env.get("pm_uptime", 0)
            if uptime_ms and pm2_env.get("status") == "online":
                uptime_sec = int((time_module.time() * 1000 - uptime_ms) / 1000)
            ...
            services.append({
                "name": name,
                "pm_id": proc.get("pm_id", pm2_env.get("pm_id", 0)),
                "status": pm2_env.get("status", "unknown"),
                "pid": proc.get("pid", 0),
                "cpu": monit.get("cpu", 0),
                "memory": round(monit.get("memory", 0) / (1024 * 1024), 1),
                "uptime": uptime_str,
                "self": name == "backend-server"
            })

        return {"success": True, "services": services}
    except subprocess.TimeoutExpired:
        raise HTTPException(status_code=504, detail="PM2 명령 시간 초과")
    except json_module.JSONDecodeError:
        raise HTTPException(status_code=500, detail="PM2 출력 파싱 실패")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"서비스 조회 실패: {str(e)}")


@app.post("/api/server/services/{pm_id}/{action}")
async def control_server_service(pm_id: int, action: str):
    """PM2 서비스 제어 (restart/stop)"""
    if action not in ("restart", "stop"):
        raise HTTPException(status_code=400, detail="허용된 액션: restart, stop")

    # 자기 자신(backend-server) 확인
    try:
        result = subprocess.run(
            ["pm2", "jlist"],
            capture_output=True, text=True, timeout=10
        ...
        target = None
        for proc in processes:
            if proc.get("pm_id", proc.get("pm2_env", {}).get("pm_id")) == pm_id:
                target = proc
                break

        if not target:
            raise HTTPException(status_code=404, detail=f"PM2 프로세스 {pm_id}를 찾을 수 없습니다")

        target_name = target.get("pm2_env", {}).get("name", target.get("name", ""))
        if target_name == "backend-server" and action == "stop":
            raise HTTPException(status_code=403, detail="백엔드 서버는 정지할 수 없습니다 (API 응답 불가)")

    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"프로세스 확인 실패: {str(e)}")

    # 액션 실행
    try:
        result = subprocess.run(
            ["pm2", action, str(pm_id)],
            capture_output=True, text=True, timeout=15
        )
        if result.returncode != 0:
            raise HTTPException(status_code=500, detail=f"PM2 {action} 실패: {result.stderr}")

        return {"success": True, "message": f"서비스 {pm_id} {action} 완료", "action": action, "pm_id": pm_id}
    except subprocess.TimeoutExpired:
        raise HTTPException(status_code=504, detail="PM2 명령 시간 초과")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"서비스 제어 실패: {str(e)}")


@app.get("/api/server/connections")
async def get_server_connections():
    """서버 접속자(established 커넥션) 수 조회"""
    try:
        result = subprocess.run(
        ...
        for line in result.stdout.strip().split("\n"):
            ...
            if len(parts) >= 4:
                local_addr = parts[3]
                if local_addr.endswith(":8000"):
                    port_8000 += 1
                elif local_addr.endswith(":3000"):
                    port_3000 += 1
        ...
        return {
            ...
            "port_8000": port_8000,
            "port_3000": port_3000,
            "total": port_8000 + port_3000
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"접속자 조회 실패: {str(e)}")


@app.get("/api/server/resources")
async def get_server_resources():
    """서버 시스템 리소스(CPU, RAM, Disk) 사용량 조회"""
    try:
        # CPU 사용률 - /proc/stat 기반 (1초 샘플링 없이 즉시 반환)
        cpu_percent = 0.0
        try:
            result = subprocess.run(
                ["top", "-bn1"],
        ...
        try:
            ...
            ram_used = ram_total - mem_available
        ...
        try:
            st = os.statvfs("/")
            disk_total = round((st.f_frsize * st.f_blocks) / (1024 ** 3), 1)  # GB
            disk_free = round((st.f_frsize * st.f_bavail) / (1024 ** 3), 1)  # GB
        ...
        raise HTTPException(status_code=500, detail=f"리소스 조회 실패: {str(e)}")
