...
let excelFieldMapping = {};
let excelCoursesOptions = '';

window.showExcelUpload = async function() {
    const div = document.getElementById('excel-upload');

    // 과정 목록 조회
    excelCoursesOptions = '<option value="">과정 선택 (선택사항)</option>';
    try {
        const response = await axios.get(`${API_BASE_URL}/api/courses`);
        response.data.forEach(course => {
            excelCoursesOptions += `<option value="${course.code}">${course.name}</option>`;
        });
    } catch (e) {
        console.error('과정 목록 조회 실패:', e);
    }
    ...
    div.innerHTML = `
        ...
        <div class="space-y-4">
            <div>
                ...
                <select id="excel-course" class="w-full px-3 py-2 border rounded-lg">
                    ${excelCoursesOptions}
                </select>
            </div>
            <div>
                <label class="block text-gray-700 mb-2">Excel 파일 선택</label>
                <input type="file" id="excel-file" accept=".xlsx,.xls" class="w-full px-3 py-2 border rounded-lg" onchange="window.previewExcelMapping()">
            </div>
            <div id="excel-mapping-area"></div>
            <div class="space-x-2">
                <button id="excel-upload-btn" onclick="window.uploadExcel()" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg" disabled>
...
window.previewExcelMapping = async function() {
    const fileInput = document.getElementById('excel-file');
    const mappingArea = document.getElementById('excel-mapping-area');
    const uploadBtn = document.getElementById('excel-upload-btn');

    if (!fileInput.files[0]) return;

    mappingArea.innerHTML = '<p class="text-gray-500">파일 분석 중...</p>';

    const formData = new FormData();
    formData.append('file', fileInput.files[0]);

    try {
        const response = await axios.post(`${API_BASE_URL}/api/students/preview-excel`, formData, {
            headers: { 'Content-Type': 'multipart/form-data' }
        });

        excelPreviewData = response.data;
        excelFieldMapping = { ...response.data.auto_mapping };

        // 매핑 UI 생성
        let mappingHtml = `
            <div class="border rounded-lg p-4 bg-gray-50">
                <h4 class="font-bold mb-3">필드 매핑 (총 ${response.data.total_rows}건)</h4>
                <p class="text-sm text-gray-600 mb-3">시스템 필드와 Excel 컬럼을 매칭해주세요. 매칭 안 된 필드는 빈칸으로 처리됩니다.</p>
                <div class="space-y-2">
        `;

        const excelColOptions = response.data.excel_columns.map(col =>
            `<option value="${col}">${col}</option>`
        ).join('');

        response.data.system_fields.forEach(field => {
            const matched = excelFieldMapping[field.key] || '';
            const matchedDisplay = matched ? 'border-green-500 bg-green-50' : 'border-orange-300 bg-orange-50';
            const requiredBadge = field.required ? '<span class="text-red-500 ml-1">*</span>' : '';

            mappingHtml += `
                <div class="flex items-center gap-2 ${matchedDisplay} border rounded p-2">
                    <span class="w-24 text-sm font-medium">${field.label}${requiredBadge}</span>
                    <span class="text-gray-400">→</span>
                    <select id="map-${field.key}" class="flex-1 px-2 py-1 border rounded text-sm" onchange="window.updateFieldMapping('${field.key}', this.value)">
        ...
        mappingHtml += `
                ...
                <div class="mt-4 p-3 bg-white border rounded">
                    <h5 class="text-sm font-bold mb-2">샘플 데이터 (상위 3건)</h5>
                    <div class="text-xs overflow-x-auto">
                        <table class="min-w-full">
                            <thead><tr class="bg-gray-100">
                                ${response.data.excel_columns.map(col => `<th class="px-2 py-1 border">${col}</th>`).join('')}
                            </tr></thead>
                            <tbody>
                                ${response.data.sample_data.map(row => `<tr>${response.data.excel_columns.map(col => `<td class="px-2 py-1 border">${row[col] || ''}</td>`).join('')}</tr>`).join('')}
                            </tbody>
                        </table>
                    </div>
                </div>
        ...
        response.data.system_fields.forEach(field => {
            ...
            if (select && excelFieldMapping[field.key]) {
                select.value = excelFieldMapping[field.key];
            }
        });
        ...
        console.error('Excel 미리보기 실패:', error);
        mappingArea.innerHTML = '<p class="text-red-500">파일 분석 실패: ' + (error.response?.data?.detail || error.message) + '</p>';
    }
}

window.updateFieldMapping = function(fieldKey, excelColumn) {
    if (excelColumn) {
        excelFieldMapping[fieldKey] = excelColumn;
    } else {
        delete excelFieldMapping[fieldKey];
    }

    // 매핑 상태에 따라 스타일 업데이트
    const container = document.getElementById(`map-${fieldKey}`).parentElement;
    if (excelColumn) {
        container.className = container.className.replace('border-orange-300 bg-orange-50', 'border-green-500 bg-green-50');
    } else {
        container.className = container.className.replace('border-green-500 bg-green-50', 'border-orange-300 bg-orange-50');
    }
}
...
window.uploadExcel = async function() {
    const fileInput = document.getElementById('excel-file');
    const courseSelect = document.getElementById('excel-course');
    ...
    // 이름 필드 매핑 필수 체크
    if (!excelFieldMapping['name']) {
        alert('이름 필드는 필수로 매핑해야 합니다.');
        return;
    }
    ...
    if (courseSelect && courseSelect.value) {
        params.append('course_code', courseSelect.value);
    }
    params.append('mapping', JSON.stringify(excelFieldMapping));

    try {
        const response = await axios.post(`${API_BASE_URL}/api/students/upload-excel?${params.toString()}`, formData, {
            headers: { 'Content-Type': 'multipart/form-data' }
        });
        ...
        if (data.success_count > 0) {
            resultMsg += `✅ 등록: ${data.success_count}명\n`;
        }

        if (data.duplicate_list && data.duplicate_list.length > 0) {
            resultMsg += `⚠️ 중복 제외: ${data.duplicate_count}명\n`;
            data.duplicate_list.forEach(d => {
                resultMsg += `   - ${d.name} (${d.phone || '전화없음'}, ${d.gender || '성별없음'})\n`;
            });
        }

        if (data.skipped_count > 0) {
            resultMsg += `⏭️ 건너뜀: ${data.skipped_count}명\n`;
        }
